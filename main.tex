...
\label{sec:orgf44845b}
\subsection*{Multi-Fidelity Learning}
\label{sec:org88e972a}
...
\label{sec:org7692a8b}
\subsubsection*{Perovskite Band Gaps}
\label{sec:orgfe1b1b9}
...
expected to aid in \hyperref[sec:org7c65217]{inverse design} and in \hyperref[sec:org3b0c070]{Feature Engineering}.
...
\citet{yang-2022-high-throug} and in \hyperref[sec:org127281b]{DFT Details}.
...
\hyperref[sec:org1f172af]{Methods}.

 
\begin{figure}[htbp]
\centering
\includesvg[width=300pt]{./.ob-jupyter/2ff00cfe78bc5c9b2f1f7e489f0bb2c6a0643052}
\caption{\label{fig:bg_dist} Variability in band gap per level of theory}
\end{figure}

\subsubsection*{Sampling}
\label{sec:orge6c15fe}
...
organic-inorganic samples. See figure \ref{fig:lot_mix_org}. See \hyperref[sec:org1f172af]{Methods}
...
perovskite property prediction. See \hyperref[sec:orge02166c]{Results and Discussion}.
...
adapter\footnote{\url{https://github.com/PanayotisManganaris/yogi}\label{org04fc3ac}}.
...
simple entropy minimization algorithm\textsuperscript{\ref{org04fc3ac}}.
...
\section*{Methods}
\label{sec:org1f172af}
\subsection*{DFT Details}
\label{sec:org127281b}
...
constituents, we provide a python tool\footnote{\url{https://github.com/PanayotisManganaris/cmcl}\label{orgefc9d72}} which robustly coverts
...
\begin{itemize}
\item generated using cmcl\textsuperscript{\ref{orgefc9d72}}
\item see \hyperref[sec:org7d6072b]{Predictor Variables}
\end{itemize}
...
every set of model parameters that is considered (See \hyperref[sec:orgbcf46e0]{Learning Curves}). Finally, the optimized model's ability to extrapolate is
...
super-space generated by combinatorial operations on the raw \hyperref[sec:org7d6072b]{Predictor Variables}.
...
\label{sec:orge02166c}
\subsection*{Best Models on Raw Domain}
\label{sec:orga256a7f}
 
\begin{multicol}
\centering
\includesvg[inkscapeformat=png, inkscapedpi=300,width=450pt]{./.ob-jupyter/e8496f3d4b7310af1c8a8f0448250398e72aed35}
\caption{\label{fig:pairplots} Multi-fidelity model predictions vs Experimental values}
\end{multicol}

\subsection*{SHAP Analysis of Domain}
\label{sec:orgbe3bcd4}
\begin{figure}[H]
\centering
...
\label{sec:org81451b1}
The scores for the nonlinear model does not indicate much use for
X-site elements. However, the B site elements contribute much more to
the band gap on average, which is consistent with our physical
understanding. Curiously, A site elements also prove to be relevant.

It is interesting to see how models make use of features in light of
said features basic bi-variate correlations

\subsection*{SISSO Model and Incidental Engineered Features}
\label{sec:org5e523c5}
The \Acrfull{sisso}
...
with the observations and model accuracy gains do not outstrip
complexity. However, In the process of creating Equation \ref{eq:bgexp},
150 SIS predictor variables were determined and recorded. These can
serve as a high quality, introspective domain for the other
architectures to fit on.

We set the aim of decreasing \(\mathcal{O}(n^3)\) computational
expense of GPR by \(\approx\)10 times. So, we aim to take 20 highly
...
problems inherent to the raw \hyperref[sec:orged853de]{Featurization of Chemistries}

Computing \ACRshort{gpr} in conjunction with SIS features, we can create
extrapolative models on a much more continuous domain.

In particular, using \ACRshort{gpr} with \ACRshort{sisso} also leverages the SIS features
...
\label{sec:org5c645dd}
\section*{Future Work}
\label{sec:org7c65217}
\subsection*{Data Science and Materials Engineering}
\label{sec:orgcf3471f}
...
\section*{Appendix}
\label{sec:orgc1e7fd3}
\subsection*{Predictor Variables}
\label{sec:org7d6072b}
