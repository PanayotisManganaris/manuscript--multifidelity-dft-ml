#+TITLE: Optimized RFR Band Gap Models based on SIS Features
...
#+begin_src jupyter-python
  cpipe = joblib.load("./Models/rfr_e_bg.joblib")
#+end_src
...
#+begin_src jupyter-python
  ...
  cpipe = mkpipe(preprocessor, SIS, RandomForestRegressor())
#+end_src
...
#+begin_src jupyter-python
  with open('./Models/RFR_e.html', 'w') as f:  
...
#+begin_src jupyter-python :file ../LearningCurves/rfr_e_bg.png
...
#+begin_src jupyter-python
  p.write_html('./LearningCurves/rfr_e_bg.html', include_plotlyjs='../plotly.min.js')
#+end_src
...
#+begin_src jupyter-python :file ../ValidationCurves/rfr_e_bg_maxlnodes.svg
  # mask = lc.score.str.contains('rmse|r2')
  # lc = lc[mask]
  p = px.line(vc, x=param_name, y='valuemean', error_y='valuestd',
              facet_col='score', facet_col_wrap=3, color='partition',
...
*** evaluate  
#+begin_src jupyter-python :file ../ParityPlots/rfr_e_bg.svg
...
#+begin_src jupyter-python 
  p.write_html('./ParityPlots/rfr_e_bg.html', include_plotlyjs='../plotly.min.js')
#+end_src
...
#+begin_src jupyter-python
  pd.concat([data, Y[['Formula', 'LoT', 'mix', 'org']]], axis=1).to_csv('~/data/perovskites/rfr_sis_pred.csv')
#+end_src
...
#+begin_src jupyter-python
  joblib.dump(cpipe, "./Models/rfr_e_bg.joblib")
#+end_src
