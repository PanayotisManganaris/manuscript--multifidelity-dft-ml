...
* COMMENT text for the body
Finally, by testing GA recommendations with DFT we form a loop that
...
Extending this inverse design framework to non-cubic Perovskite phases
is likely to be best enabled by the sophisticated graph neural
networks being developed for exactly this purpose.

Surely, this will be aided by the growing databases of experimentally
measured Perovskite compounds with well defined structures.

In fact, it is expected that these novel assets will even permit
reasonable multi-fidelity models to be developed under the graph
learning paradigm.
