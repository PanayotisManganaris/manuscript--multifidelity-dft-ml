...
- SOC is performed for better electronic properties.
...
#+begin_src jupyter-python :exports both
  df = Y.Formula[[25,45]].to_frame().ft.comp()
  df.index = Y.Formula[[30,45]]
  df
#+end_src
...
|--------------------------------+----+-----+-------+-------+-----|
| Formula                        |    |     |       |       |     |
| CsCa0.125Ba0.125Ge0.25Sn0.5Br3 | 1  | 1.0 | 0.375 | 2.625 | NaN |
| CsCaCl_1.875_I_1.125           | 1  | NaN | 1.125 | 1.875 | 1.0 |
:end:
...
- 14 sparse composition vectors extracted from chemical formula
  ...
  - see Appendix [[Predictor Variables]] 
...
- 5 categorical dimensions one-hot-encoding [[gls:lot][level of theory]].
  - this provides the domain-side categorical axis for [[glssymbol:mtl][multi-task
    learning]]
  - see table ref:tbl:LoTs
...
the [[acrshort:gpr][GPR]] a principled linear model.

We hope [[Acrfull:shap][Shapley Additive Explaination (SHAP)]] analysis of the models
...
1) generality over the domains of Perovskite compositions and
...
explain this high entropy mixing diversity by utilizing the [[glssymbol:cmix][cardinal
...
[[Learning Curves]]). Finally, the optimized model's ability to
extrapolate is tested by training/testing on splits determined with a
[[gls:gkf][groupwise K-fold]] splitting strategy.
...
of the [[Acrfull:sisso][Sure Independence Screening and Sparsifying Operator
(SISSO)]][[cite:&ouyang-2018-sisso]].

SIS is a powerful application od the remarkable Artificial
Intelligence technique, compressed
...
predictors (Appendix [[Predictor Variables]]).
...
#+begin_src jupyter-python :post wrap(*this*, w="300pt :float multicol", c="label:fig:pairplots Multi-fidelity model predictions vs Experimental values")
...
#+attr_latex: :width 300pt :float multicol
#+CAPTION: label:fig:pairplots Multi-fidelity model predictions vs Experimental values
[[file:./.ob-jupyter/e8496f3d4b7310af1c8a8f0448250398e72aed35.svg]]
:end:

** TODO Domain analysis
[[Acrfull:shap][Shapley Additive Explaination (SHAP)]]
scores[[cite:&lundberg-2017-unified-approac]] are computed
 
*** TODO SHAP analysis
*** Random Forest Band Gap SHAP Values
:PROPERTIES:
:BEAMER_env: block
:END:
#+attr_latex: :width 0.25\paperwidth
...
*** Gaussian Process Band Gap SHAP Values                         :B_block:
:PROPERTIES:
:BEAMER_env: block
:END:
#+attr_latex: :width 0.25\paperwidth
...
#+begin_src jupyter-python
  sub = pearson.iloc[[56, 59], 0:55]
  ytitles = ["Band Gap [eV]", "Decomposition Energy [eV]"]
  sub = sub.loc[:,(sub.abs()>0.5).sum(axis=0).apply(bool)]
#+end_src
...
*** TODO engineered features for Enhanced Correlation
...
The SIS operator implemented by the [[ACRshort:sisso][SISSO]] code[[cite:&ouyang-2018-sisso]]
...
Computing [[ACRshort:gpr][GPR]] in conjunction with SIS features, we can create much
models on a much denser domain, which gives many advantages in
enabling more interpolation.

In particular, using [[ACRshort:gpr][GPR]] with [[ACRshort:sisso][SISSO]] also leverages the SIS features
...
** TODO COMMENT HPO Summary Tables
*** RFR
*** GPR
...
| sisso | SISSO        | Sure Independence Screening and Sparsifying Operator[fn:1] |
